...
export function Home() {
    ...
    const [loadoImages, setLoadImages] = useState<string[]>([]);
                ...
                {cars.map(car => (
                    <Link key={car.id} to={`/car/${car.id}`}>
                        <section className="w-full bg-white rounded-lg">
                            <div className="w-full h-72 rounded-lg bg-slate-200" style={{ display: loadoImages.includes(car.id) ? 'none' : 'block' }}></div>
                            <img
                                className="w-full rounded-lg mb-2 max-h-72 hover:scale-105 transition-all cursor-pointer"
                                src={car.images[0].url}
                                alt="Carro"
                                onLoad={() => handleImageLoad(car.id)}
                                style={{ display: loadoImages.includes(car.id) ? 'block' : 'none' }}
                            />
